...
with open(census_csv) as csvfile:
    ...
    for row in csvreader:
        ...
        poverty_percent = round(100*int(row[8])/int(row[1]),2)
        poverty_rate.append(str(poverty_percent))
        # Split the place into county and state
        split_place = row[0].split(",")
        county.append(split_place[0])
        state.append(split_place[1])
# Zip lists together
zipped_list = zip(place, population, income, poverty_count, poverty_rate, county, state)
